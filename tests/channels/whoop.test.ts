...
describe('processRecords', () => {
  it('delivers recovery message to main group JID', () => {
    const onMessage = vi.fn();
    const channel = new WhoopChannel(makeOpts({
      onMessage,
      registeredGroups: () => ({
        'tg:main123': { name: 'main', folder: '/tmp/main', trigger: '', added_at: '', isMain: true },
      }),
    }));

    const records = [{
      cycle_id: 100, sleep_id: 'sleep-uuid', user_id: 1,
      created_at: '2026-04-12T01:00:00Z', updated_at: '2026-04-12T05:00:00Z',
      score_state: 'SCORED',
      score: { user_calibrating: false, recovery_score: 72, resting_heart_rate: 58, hrv_rmssd_milli: 80.5, spo2_percentage: 96.0, skin_temp_celsius: 33.5 },
    }];

    (channel as any).processRecords('recovery', records, (r: any) => r.cycle_id.toString());
    expect(onMessage).toHaveBeenCalledTimes(1);
    const call = onMessage.mock.calls[0];
    expect(call[0]).toBe('tg:main123');
    expect(call[1].id).toBe('whoop:recovery:100');
    expect(call[1].sender_name).toBe('WHOOP');
    expect(call[1].content).toContain('[WHOOP Recovery]');
    expect(call[1].content).toContain('Score: 72%');
  });

  it('skips PENDING_SCORE records', () => {
    const onMessage = vi.fn();
    const channel = new WhoopChannel(makeOpts({
      onMessage,
      registeredGroups: () => ({
    ...
    (channel as any).processRecords('recovery', [{ cycle_id: 200, score_state: 'PENDING_SCORE' }], (r: any) => r.cycle_id.toString());
    expect(onMessage).not.toHaveBeenCalled();
  });

  it('skips already-processed IDs', () => {
    const onMessage = vi.fn();
    const channel = new WhoopChannel(makeOpts({
      onMessage,
      registeredGroups: () => ({
    ...
    const records = [{
      cycle_id: 300, sleep_id: 's', user_id: 1,
      created_at: '2026-04-12T01:00:00Z', updated_at: '2026-04-12T05:00:00Z',
      score_state: 'SCORED',
      score: { user_calibrating: false, recovery_score: 50, resting_heart_rate: 60, hrv_rmssd_milli: 40, spo2_percentage: 95, skin_temp_celsius: 33 },
    }];
    (channel as any).processRecords('recovery', records, (r: any) => r.cycle_id.toString());
    (channel as any).processRecords('recovery', records, (r: any) => r.cycle_id.toString());
    expect(onMessage).toHaveBeenCalledTimes(1);
  ...
  it('skips when no main group registered', () => {
    const onMessage = vi.fn();
    const channel = new WhoopChannel(makeOpts({ onMessage, registeredGroups: () => ({}) }));
    const records = [{
      cycle_id: 400, sleep_id: 's', user_id: 1,
      created_at: '2026-04-12T01:00:00Z', updated_at: '2026-04-12T05:00:00Z',
      score_state: 'SCORED',
      score: { user_calibrating: false, recovery_score: 90, resting_heart_rate: 50, hrv_rmssd_milli: 100, spo2_percentage: 97, skin_temp_celsius: 33 },
    }];
    (channel as any).processRecords('recovery', records, (r: any) => r.cycle_id.toString());
    expect(onMessage).not.toHaveBeenCalled();
  });
